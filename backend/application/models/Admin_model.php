...
class Admin_model extends CI_Model {
	...
	public function get_users() {
		...
		foreach ( $this->db->get ( 'user' )->result () as $row ) {
			$result [] = array (
					...
					'time' => $this->date_diff ( $row->lastModifiedDate ) 
			);
		}
		return $result;
	}
	public function get_products() {
		...
		foreach ( $this->db->get ( 'product' )->result () as $row ) {
			$result [] = array (
					...
					'price' => $row->price,
					'desc' => unserialize ( $row->desc )->details,
	...
	public function getOrders() {
		$this->db->select ( 'order.order_id, order.quantity, order.price, order.orderStatus, order.orderDate, product.product_name, user.firstname, user.lastname' );
		$this->db->join ( 'product', 'product.product_id = order.product_id' );
		$this->db->join ( 'user', 'user.user_id = order.user_id' );
		$this->db->order_by ( 'order.lastModifiedDate', 'DESC' );
		$result = array();
		foreach ( $this->db->get ( 'order' )->result () as $row ) {
				$result[]= array('user_name'=> $row->firstname.' '.$row->lastname,'order_id' => $row->order_id, 'product_name' =>$row->product_name, 'orderStatus'=> $row->orderStatus, 'quantity' => $row->quantity, 'price' => $row->price, 'date' => $row->orderDate);
		}
		return $result;
	}
